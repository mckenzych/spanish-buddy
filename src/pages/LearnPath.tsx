...
export default function LearnPath() {
      ...
      try {
        ...
        const unitsWithLessons: Unit[] = (unitsData || []).map((unit) => {
          ...
          return {
            ...unit,
            lessons: unitLessons,
            completedCount: unitLessons.filter((l) => l.completed).length,
          };
        });
  ...
  return (
    <div className="container py-8">
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold md:text-4xl">Learning Path</h1>
        <p className="mt-2 text-muted-foreground">
          Master Spanish step by step with structured lessons
        </p>
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        {units.map((unit, index) => {
          const progress = unit.lessons.length > 0 
              ...
              <CardHeader className="pb-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <CardTitle className="font-display text-xl">{unit.title}</CardTitle>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{unit.completedCount}/{unit.lessons.length} lessons</span>
                  </div>
                ...
                <div className="space-y-2">
                  {unit.lessons.map((lesson, lessonIndex) => {
                    ...
                    return (
                      <Link
                        key={lesson.id}
                        to={isLocked || lessonLocked ? "#" : `/lesson/${lesson.id}`}
                        className={isLocked || lessonLocked ? "pointer-events-none" : ""}
                      >
                        <div 
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <span className={`font-medium ${
                              isLocked || lessonLocked ? "text-muted-foreground" : ""
                            }`}>
                              {lesson.title}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            {!isLocked && !lessonLocked && !lesson.completed && (
                ...
                {/* Start button */}
                {!isLocked && (
                  <Link to={`/lesson/${unit.lessons[0]?.id}`}>
                    <Button className="w-full gap-2" disabled={!unit.lessons.length}>
                      {isCompleted ? "Review Unit" : unit.completedCount > 0 ? "Continue" : "Start Unit"}
