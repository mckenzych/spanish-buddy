...
export default function Practice() {
  ...
  useEffect(() => {
    if (mode === "multiple-choice" && vocabulary.length > 0) {
      generateChoices();
    }
  ...
  const generateChoices = () => {
    ...
    const allChoices = [...wrongChoices, current.spanish].sort(() => Math.random() - 0.5);
  ...
  return (
    <div className="container py-8 max-w-2xl">
      <div className="mb-6">
        <h1 className="font-display text-3xl font-bold">Practice Hub</h1>
        <p className="text-muted-foreground">Strengthen your vocabulary with different exercises</p>
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Button variant="outline" size="sm" onClick={shuffleWords} className="gap-2">
      ...
      <Tabs value={mode} onValueChange={(v) => setMode(v as PracticeMode)}>
        ...
        <TabsContent value="flashcards">
          <Card 
            className={`cursor-pointer transition-all duration-300 min-h-[300px] ${
          ...
          >
            <CardContent className="flex flex-col items-center justify-center p-8 text-center min-h-[300px]">
              {!showAnswer ? (
                <>
                  <p className="text-sm text-muted-foreground mb-2">Translate to Spanish:</p>
                ...
                <>
                  <p className="text-sm text-muted-foreground mb-2">Spanish:</p>
                  <p className="font-display text-3xl font-bold text-primary">{currentWord.spanish}</p>
                  {currentWord.pronunciation_hint && (
                    <p className="mt-2 text-muted-foreground">[{currentWord.pronunciation_hint}]</p>
                  )}
                  <Button 
                    variant="ghost" 
                    size="sm" 
          ...
          <Card className="min-h-[300px]">
            <CardHeader>
              <CardTitle className="text-center">
                What is "{currentWord.english}" in Spanish?
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {choices.map((choice, i) => (
                <Button
                  ...
                  variant={
                    feedback
                      ? choice === currentWord.spanish
                        ? "default"
                        : "outline"
                      : "outline"
                  }
                  className={`w-full justify-start text-left h-auto py-4 ${
                    feedback && choice === currentWord.spanish 
              ...
              {feedback === "incorrect" && (
                <div className="text-center mt-4">
                  <p className="text-destructive mb-2">Try again!</p>
        ...
        <TabsContent value="typing">
          <Card className="min-h-[300px]">
            <CardHeader>
              <CardTitle className="text-center">
                Type "{currentWord.english}" in Spanish
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {feedback === "correct" && (
                <div className="text-center animate-scale-in">
                  ...
                  <p className="text-primary font-medium">¡Correcto!</p>
                </div>
              )}
              ...
              {feedback === "incorrect" && (
                <div className="text-center">
                  ...
                  <p className="text-destructive font-medium mb-2">Not quite...</p>
                  <p className="text-muted-foreground">
                    Correct answer: <span className="font-medium text-primary">{currentWord.spanish}</span>
                  </p>
                  <Button onClick={nextWord} className="mt-4">Continue</Button>
                </div>
              )}
              ...
              {!feedback && (
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={() => toast({ title: "Hint", description: currentWord.pronunciation_hint || "No hint available" })}
                  >
            ...
            <CardContent className="text-center space-y-6">
              <div>
                ...
                <Button 
                  ...
                  size="sm" 
                  className="gap-2"
                  onClick={() => playAudio(currentWord.spanish)}
                >
                  <Volume2 className="h-4 w-4" />
                  Hear Spanish
                </Button>
              ...
              <div className="bg-muted/50 rounded-lg p-4">
                ...
                {currentWord.pronunciation_hint && (
                  <p className="text-sm text-muted-foreground mt-1">[{currentWord.pronunciation_hint}]</p>
                )}
              </div>
            </CardContent>
          </Card>
      ...
      <div className="flex justify-between mt-6">
        <Button variant="outline" onClick={prevWord} className="gap-2">
          <ChevronLeft className="h-4 w-4" />
          Previous
        </Button>
        <Button variant="outline" onClick={() => { setShowAnswer(false); setFeedback(null); }} className="gap-2">
